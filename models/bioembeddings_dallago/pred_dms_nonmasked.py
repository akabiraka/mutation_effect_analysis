...
def execute(protid_list):
    ...
    for i, protid in enumerate(protid_list):
        ...
        output_logits = model_utils.compute_model_logits(
            model, protid, seq, model_logits_out_dir
        )
        preds_related_to_aprot = model_utils.compute_variant_effect_scores(
            variants_df, tokenizer, protid, output_logits, model.aa_prefix
        )
