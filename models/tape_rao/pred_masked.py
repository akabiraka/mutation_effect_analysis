...
from models.aa_common.data_loader import (
...
model_name = "protbert"  # unirep, protbert. unirep may work, but did not use unirep from here see jax_unirep
...
def execute(protid_mutpos_tuple_list):
    ...
    for i, (protid, mut_pos) in enumerate(protid_mutpos_tuple_list):
        ...
        output_logits = model_utils.compute_model_logits_from_masked_sequences(
            model, tokenizer, protid, seq, mut_pos, model_logits_out_dir
        )
        preds_related_to_aprot = (
            model_utils.compute_variant_effect_scores_from_masked_logits(
                variants_df, tokenizer, protid, mut_pos, output_logits
            )
        )
...
if __name__ == "__main__":
    protid_mutpos_df = variants_df[
        ["prot_acc_version", "1indexed_prot_mt_pos"]
    ].drop_duplicates(
        keep="first"
    )  # protid_mutpos_seq_df
    ...
    result_df.to_csv(
        f"{model_task_out_dir}/preds_{model_name}_masked.tsv",
        sep="\t",
        index=False,
        header=True,
    )
